...
if ($username&&$password) 
{
	...
	mysqli_select_db($conn, "farmasi") or die("incorrect database");

	$query = mysqli_query($conn, "SELECT * FROM user WHERE username= '$username'");//syntax error mysqli//
	...
	if($numrows!==0)
	{
		...
		if($username==$dbusername && $password==$dbpassword)
		{
			...
			if($dblevel == 1)
			{
				...
				header("location: user.php");// kesalahan ejaan file location//
		}
		else
			?>
